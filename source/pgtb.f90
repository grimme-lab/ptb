...
&               pnt,norm,S,T,D,efield,qeeq,S1,S2,psh,pa,&
...
!  real(wp),parameter :: cok   = 1.00_wp  ! OK mixing in 1. iter, not as important as in second iter
...
subroutine momatch(pr,ndim,nocc,nvirt,S)
      ...
      real*8,parameter :: lumoweight = 1d0/3d0  ! LUMO weight for fit
...
!     write(133,'(6f11.4)') epsref(nocc+1),e(nocc+1)
      ...
      if(gap_ref.lt.15d0) then ! only reasonable gaps included in fit
         do j=nocc+1,nocc+2
