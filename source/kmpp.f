...
*  use sm_module 
       IMPLICIT NONE
       INTEGER(8) P, N, K, Z, IFAULT
       REAL(8) X, C, WORK
       DIMENSION X(P,N), C(P,K), Z(N), WORK(N)
 
*               constants
       INTEGER(8) ITER                 ! maximum iterations
       REAL(8) BIG                     ! arbitrary large number
       PARAMETER (ITER = 1000,
     $            BIG = 1E33)
...
*                local variables
     ...
     $      W                     ! temp scalar
       ...
       LOGICAL(4) CHANGE             ! whether any points have been reassigned
...
*             find nearest center for each point 
         DO I = 1, N
           ...
           DO L = 1, K
             D2 = 0.
             IF (D2 .GE. BEST) GO TO 30               ! needless to add to D2
             DO J = 1, P
               D2 = D2 + (X(J,I) - C(J,L)) **2
